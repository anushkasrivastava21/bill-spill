...
export default async function DashboardLayout({ children }) {
  ...
  return (
    <div className="min-h-screen flex bg-background">
      {/* Desktop SideNav */}
      <nav className="hidden md:flex flex-col p-md gap-base bg-surface-container-low h-screen w-64 left-0 top-0 fixed shadow-xl z-50">
        ...
        <div className="flex flex-col gap-sm flex-1">
          <Link href="/dashboard/expenses" className="flex items-center gap-sm px-4 py-3 text-on-surface-variant hover:bg-surface-container-highest rounded-xl transition-colors font-label-md">
            <span className="material-symbols-outlined">payments</span>
            My Expenses
          </Link>
          <Link href="/dashboard/groups" className="flex items-center gap-sm px-4 py-3 text-on-surface-variant hover:bg-surface-container-highest rounded-xl transition-colors font-label-md">
        ...
        <form action="/auth/signout" method="post" className="mt-auto">
          <button type="submit" className="flex w-full items-center gap-sm px-4 py-3 text-on-surface-variant hover:bg-error-container hover:text-error rounded-xl transition-colors font-label-md cursor-pointer">
      ...
      {/* TopAppBar */}
      <header className="fixed top-0 w-full z-40 flex justify-between items-center px-container-margin h-16 bg-surface/80 backdrop-blur-md border-b border-white/20 shadow-sm md:ml-64 md:w-[calc(100%-16rem)]">
      ...
      {/* Main Content */}
      <main className="flex-1 w-full md:ml-64 pt-20 pb-24 md:pb-10 px-container-margin md:px-lg max-w-[1200px] mx-auto min-h-screen">
        {children}
      </main>
